...
const Fase3Assessor = (props) => {
    ...
    const proximaFase = async () => {
        ...
        const response = await request.verifyAssessorEmpresaEmailAndCnpj(assessorFields).then()        
    ...
    const cnpjMask = (e) => {        
        var replacedValue = e.target.value.replace(/\D/g, '').match(/(\d{0,2})(\d{0,3})(\d{0,3})(\d{0,4})(\d{0,2})/);
        e.target.value = !replacedValue[2] ? replacedValue[1] : replacedValue[1] + '.' + replacedValue[2] + '.' + replacedValue[3] + '/' + replacedValue[4] + (replacedValue[5] ? '-' + replacedValue[5] : '');
        cnpj.current.style.borderColor = `black`
        if (e.target.value) {
            ...
            <div className={styles.fase3_content}>
                <div className={styles.fase3_body}>
                    <div className={styles.fase3_body_input}>
                        <label htmlFor="email">Qual e-mail da sua empresa?</label>
                        <input type="text" placeholder="Email" id="email" defaultValue={props.usuario.emailEmpresa} ref={email} onInput={isEmailValid}/>
                    </div>
                    <div className={styles.fase3_body_input}>
                        <label htmlFor="cnpj">Qual o cnpj da sua empresa?</label>
                        <input type="text" placeholder="Cnpj" id="cnpj" defaultValue={props.usuario.cnpj} ref={cnpj} onInput={e => cnpjMask(e)}/>
                    </div>
                </div>
