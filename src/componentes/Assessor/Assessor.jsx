...
function Assessor({ assessorResponseDto, orcamento, setOrcamento }) {
  ...
  const vinculateAssessorToWedding = async () => {
    const assessor = await request.vinculateAssessorToWedding(2, assessorSelected.id);
  ...
  const updateAssessorPrice = async (value) => {
    ...
    orcamento.orcamentoGasto = orcamento.orcamentoGasto - assessorPrice + newPrice;
    ...
    <div className={styles.container}>
      ...
      <div className={styles.assessorCard}>
        ...
        <div className={styles.textos}>
          ...
          <a href="#" className={styles.gerenciar} onClick={() => abrirModal()}>
            Adicionar
          </a>
        </div>
      </div>
      ...
      <div className={styles.chosen_category}>
        <div className={styles.chosen_header}>
          <div>Reservado</div>
          <FontAwesomeIcon icon={faEllipsis} onClick={abrirModal} style={{cursor: "pointer"}}/>
        </div>
        <div className={styles.chosen_body}>
          <div>
            ...
            <span className={styles.chosen_body_preco}><span>R$</span> <input onBlur={(e) => updateAssessorPrice(e.target.value)} type="number" defaultValue={assessorResponseDto.preco}/></span>
          </div>
        </div>
      </div>
      }
      {modalAberto && (
        <Modal>
          <ModalHeader onClose={fecharModal}>
            <span>{isChosen ? "Editar" : "Adicionar"} Assessor </span>
          </ModalHeader>
          <ModalBody>
            <div className={styles.containerModal}>
              ...
              <div className={styles.assessor_modal} ref={assessorModalRef}>
                {assessoresModal?.map(assessor => {
                  return <div value={assessor.id} onClick={(e) => selectAssessor(assessor, e)}>{assessor.nome}</div>
                })}
              </div>
            </div>
          </ModalBody>
          <ModalFooter>
            <ModalFooterButton
              ...
              onClick={fecharModal}
            />
            <ModalFooterButton button="add_button" text="Adicionar" onClick={vinculateAssessorToWedding}/>
          </ModalFooter>
        </Modal>
      )}
    </div>
